...
// If using the Webflow loader, you DON'T need to paste this manually (it can be auto-loaded on /kalkulator).

(function () {
  'use strict';

  // Prevent double execution
  if (window.__KLARPAKKE_CALCULATOR__) return;
  window.__KLARPAKKE_CALCULATOR__ = true;

  // Config: Realistic annual returns per plan
  const PLANS = {
    paper: { name: 'Gratis (Paper)', annual: 0, color: '#4CAF50' },
    safe: { name: 'SAFE $49', annual: 0.1, color: '#4CAF50' },
  ...
  onReady(function init() {
    // Get elements (IDs must match Webflow)
    ...
    if (!startInput || !cryptoSlider || !planSelect || !resultTable) {
      console.warn('[Klarpakke] Calculator elements not found. Check IDs.');
      return;
    }
    ...
    cryptoSlider.addEventListener('input', function () {
      if (cryptoLabel) cryptoLabel.textContent = cryptoSlider.value + '%';
      calculate();
    });
