-- Klarpakke Database Schema
-- Cloud-native: Webflow + Supabase + Make
...
-- ============================================
CREATE TABLE IF NOT EXISTS daily_risk_meter (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Public read for dashboard (anon key kan lese)
...
CREATE POLICY "Allow public read risk" 
  ON daily_risk_meter FOR SELECT 
  USING (true);
...
-- Insert/update via Make (secret key only)
...
CREATE POLICY "Allow insert risk_meter" 
  ON daily_risk_meter FOR INSERT 
  WITH CHECK (true);

CREATE POLICY "Allow update risk_meter" 
  ON daily_risk_meter FOR UPDATE 
  USING (true);
...
-- Update risk meter automatisk når position endres
...
-- ============================================
-- INITIAL DATA: Seed risk meter for today
-- ============================================
...
-- ============================================
-- COMMENTS: Dokumentasjon
-- ============================================
COMMENT ON TABLE positions IS 'Trading posisjoner (åpne og lukkede)';
COMMENT ON TABLE signals IS 'AI-genererte kjøp/salg signaler fra Perplexity';
COMMENT ON TABLE daily_risk_meter IS 'Daglig oversikt over total risiko';
COMMENT ON TABLE ai_calls IS 'Log av alle Perplexity API-kall (cost tracking)';
